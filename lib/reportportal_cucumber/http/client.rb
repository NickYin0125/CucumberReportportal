...
module ReportportalCucumber
  module Http
    # Shared HTTP client for all ReportPortal requests, including retry/backoff.
    class Client
      ...
      def post_json(path:, body:, headers: {})
        request(method: :post, path: path, body: JSON.generate(body), headers: default_headers.merge(headers).merge("Content-Type" => "application/json"))
      end
      ...
      def put_json(path:, body:, headers: {})
        request(method: :put, path: path, body: JSON.generate(body), headers: default_headers.merge(headers).merge("Content-Type" => "application/json"))
      end
      ...
      def with_http(uri)
        ...
        klass.start(uri.host, uri.port, use_ssl: uri.scheme == "https", open_timeout: @config.open_timeout,
                    read_timeout: @config.read_timeout, write_timeout: @config.write_timeout) do |http|
          yield http
        end
      end
      ...
      def encode_multipart(parts, boundary)
        ...
        parts.each do |part|
          ...
          lines << "Content-Type: #{part.fetch(:content_type, 'application/octet-stream')}\r\n\r\n"
          lines << part.fetch(:body)
          lines << "\r\n"
        end
